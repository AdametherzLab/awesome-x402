...
- [ReportCheck](https://reportcheck.adametherzlab.com) - Deterministic citation audit for AI-drafted engineering inspection reports. Validates claims against API 510 (pressure vessels), API 570 (piping), and OSHA 1910.119 PSM. Detects `missing_citation`, `wrong_paragraph` (Jaccard token overlap vs canonical text), and `unit_mismatch` — no LLM judges the citations; every flagged finding surfaces both the paragraph the report cited and the canonical paragraph the rule engine expected. NIM-only privacy: customer report text never leaves the box, never streamed to OpenRouter/Sonar, write-then-delete <60s. $49 USDC per audit on Base via Coinbase CDP facilitator. ([POST /api/audit](https://reportcheck.adametherzlab.com/api/audit)) ([healthz](https://reportcheck.adametherzlab.com/healthz))
- [Land3D](https://land3d.adametherzlab.com) - Screening-quote API for rural-Texas land 3D Gaussian Splatting walkthroughs. Submit acreage + lat/lon; returns lat-based terrain class screen, East-Texas reach-feasibility note, and next-step pricing tier (drone capture is case-by-case at v0.1, not committed). $50 USDC per quote on Base via Coinbase CDP facilitator. ([POST /api/land3d/quote](https://x402.adametherzlab.com/api/land3d/quote)) ([Marketing landing](https://land3d.adametherzlab.com))
- [PlantRisk](https://plantrisk.adametherzlab.com) - OSHA 1910.119 PSM + EPA enforcement scoreboard for industrial chemical-process facilities. 640 facilities seeded (CA, LA, NJ, TX — 160 each); 3 fully scored with real EPA case IDs sourced from EPA ECHO. Paid CSV/JSON export endpoint for agents and consultants needing site-level compliance intel. $5 USDC per export on Base via Coinbase CDP facilitator. ([POST /api/export](https://plantrisk.adametherzlab.com/api/export))
...
- [AdametherzLab x402 API Network](https://x402.adametherzlab.com) — 46 paid x402 endpoints across 8 verticals on Base mainnet via CDP facilitator. Verticals: Marketplace Analytics (14), Industrial Compliance (6 — API 571 corrosion, EPA TRI, facility risk), Permaculture (5), Commerce (5), Homestead (5), Energy/Environmental (4), Agriculture (4), Supply Chain (3). Product hubs with per-vertical cURL examples and pricing: [DataForge](https://dataforge.adametherzlab.com) (web scraping), [ComplianceOS](https://x402.adametherzlab.com/industrial), [GardenIQ](https://x402.adametherzlab.com/permaculture), [MarketLens](https://x402.adametherzlab.com/commerce), [FarmStack](https://x402.adametherzlab.com/agriculture), [GreenLedger](https://x402.adametherzlab.com/energy), [FlowOps](https://x402.adametherzlab.com/supplychain). $0.002–$50 USDC per call (Land3D screening quote at the high end; web scraping at the low end). Sister services on separate domains: [ReportCheck](https://reportcheck.adametherzlab.com) ($49, engineering report citation audit), [PlantRisk](https://plantrisk.adametherzlab.com) ($5, OSHA PSM + EPA enforcement scoreboard), [Land3D](https://land3d.adametherzlab.com) ($50, rural-TX 3D Gaussian Splatting screening quote). Discovery: [llms.txt](https://x402.adametherzlab.com/llms.txt) | [OpenAPI](https://x402.adametherzlab.com/openapi.json) | [x402 Manifest](https://x402.adametherzlab.com/.well-known/x402-manifest.json) (with `sisterServices`). ([GitHub](https://github.com/AdametherzLab))
